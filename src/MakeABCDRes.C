...
void MakeABCDRes(){
    setTDRStyle();
    TFile*f=new TFile("ALPHABETRes_V17bkg.root", "READ");
    ...
    h_A1_ZJets->Scale(scale);
    h_A1_WJets->Scale(scale);
    h_A1_QCD->Scale(scale);
    h_A1_TT->Scale(scale);
    h_A1_QCD->SetFillColor(kGray);
    h_A1_TT->SetFillColor(kCyan);
    h_A1_ZJets->SetFillColor(kGreen+2);
    h_A1_WJets->SetFillColor(kBlue);
    BackgroundStack3b->Add(h_A1_ZJets);
    BackgroundStack3b->Add(h_A1_WJets);
    BackgroundStack3b->Add(h_A1_TT);
    BackgroundStack3b->Add(h_A1_QCD);
    ...
    //I need to save 4b and 3b separately, and then added together as signal, right?
