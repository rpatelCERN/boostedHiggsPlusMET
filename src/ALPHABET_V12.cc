...
int main(int argc, char** argv){
  ...
  if (looseCuts ){
    ...
    if (region == 0 ){
      baselineCuts.push_back(*boostedBaselineCut_V12<RA2bTree>);
    }
    else if (region == 1){
  ...
  for (int i = 0 ; i < numMETbins_V12 ; i++){
    TString tag="_";
    tag+=lowestMET_V12+i*binWidth_V12;
  ...
  // regionName = "_resVeto";
  outputFile = new TFile("ALPHABETBoost"+Year+"_V12"+regionName+".root","RECREATE");
  // outputFile = new TFile("ALPHABETBoost_V12_signalOnly.root","RECREATE");
    ...
    for (int iEvt = 0 ; iEvt < numEvents ; iEvt++){
      ...
      for (int iBin = 0 ; iBin < numMETbins_V12 ; iBin++){
        if (ntuple->MET > lowestMET_V12 ){
          if (ntuple->MET > numMETbins_V12*(binWidth_V12-1)+lowestMET_V12 )
      ...
      if (doubleMassCut_V12(ntuple) ){ //this requires both lead and sublead jet to be in the mass window
      ...
      else if (!doubleMassCut_V12( ntuple ) ){
