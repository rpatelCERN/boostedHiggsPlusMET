...
int main(int argc, char** argv) {
  ...
  for ( int iSample = 0 ; iSample < skims.ntuples.size() ; iSample++) {
    ...
    float bbtagCut = 0.70; //0.89 for medium
		bool runRealDeepBB = false;
    ...
    for ( int iEvt = 0 ; iEvt < numEvents ; iEvt++ ) {
      ...
      if ( boostedBaselineCut(ntuple) ) {
        ...
        double J1_doubleB, J2_doubleB;

				if (runRealDeepBB && ntuple->JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb->size()>0){ //use REAL deep bbtag
					J1_doubleB = ntuple->JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb->at(0);
        ...
        bool J1_isBoost_antiB = (J1_pT>300.0 && J1_softDropMass>=85.0 && J1_softDropMass<135.0 && J1_doubleB<=bbtagCut);
        bool J2_isBoost_antiB = (J2_pT>300.0 && J2_softDropMass>=85.0 && J2_softDropMass<135.0 && J2_doubleB<=bbtagCut);
