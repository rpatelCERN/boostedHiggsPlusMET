...
//"clean" does not exist in RA2b tree V16 and up
...
template<typename ntupleType> bool FiltersCut(ntupleType* ntuple){
  return
      (ntuple->HBHENoiseFilter==1 //this kills no events
      && ntuple->HBHEIsoNoiseFilter==1 //this kills no events
      && ntuple->eeBadScFilter==1 //this kills no events
      //&& ntuple->EcalDeadCellTriggerPrimitiveFilter == 1 //this kills 27% of events
      && ntuple->NVtx>0 //this kills no events
      && ntuple->MET/ntuple->CaloMET < 5. //this kills no events
      && ntuple->BadPFMuonFilter == 1 //this kills 4% of events
      && ntuple->BadChargedCandidateFilter == 1 //this kills no events
...
template<typename ntupleType> bool METHTlooseCut(ntupleType* ntuple){
  return ( ntuple->MET > 250. && ntuple->HT > 300. );
}
